...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const prompt = `You are a STRICT photo restoration damage inspector. Evaluate ONLY remaining restoration damages in the provided restored image and output STRICT JSON ONLY.\n
Definitions:\n
...
Return keys:\n
- overall_quality_score: number (0-100) where 100 = no structural_damage visible; THIS SCORE MUST NOT penalize minor non_structural_quality issues.\n- confidence: number (0-1) indicating your confidence in the assessment.\n- damage_categories: array of { name: string; severity: number (0-1); confidence: number (0-1); notes?: string } listing ONLY structural_damage types you see (exclude noise/blur/compression categories).\n- recommended_second_pass_prompt: string (one concise sentence focused on removing remaining structural_damage conservatively while preserving identity).\n
Guidance:\n
